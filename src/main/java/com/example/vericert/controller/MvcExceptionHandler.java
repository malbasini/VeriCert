...
public class MvcExceptionHandler {
    ...
    public String notFound(EntityNotFoundException ex, Model model){
        ...
        return "error/error"; // es. templates/error/error.html
    }

    @ExceptionHandler(org.springframework.security.access.AccessDeniedException.class)
    @ResponseStatus(HttpStatus.FORBIDDEN)
    public String accessDenied(org.springframework.security.access.AccessDeniedException ex,
                               Model model) {
        model.addAttribute("status", 403);
        model.addAttribute("title", "Accesso negato");
        model.addAttribute("message", "Non hai i permessi per accedere a questa pagina.");
        return "error/403"; // ✅ usa il template 403 dedicato
    }
